...
export const holidayConfigApi = {
  page: (data: SysHolidayConfigPageReq) => request.post('/sys-holidays/page', data),
  detail: (id: number) => request.get(`/sys-holidays/detail/${id}`),
  options: () => request.get('/sys-holidays/options'),
  add: (data: Partial<SysHolidayConfig>) => request.post('/sys-holidays/add', data),
  update: (data: Partial<SysHolidayConfig> & { id: number }) => request.post('/sys-holidays/update', data),
