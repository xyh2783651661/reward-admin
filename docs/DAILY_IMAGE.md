...
```js
// 方式一：a 标签
<a href="/daily-images/{id}/download" download>下载</a>
...
5. **单个删除**：`DELETE /daily-images/{id}`，删除后列表需刷新
6. **批量删除**：`DELETE /daily-images/batch`，请求体为 ID 数组，返回成功删除数量
7. **缩略图回退**：后端会自动回退到原图，前端无需判断 `thumbnailPath` 是否存在
8. **source 筛选**：分页接口支持 `source` 参数精确筛选，可选值 `unsplash` / `local` / `upload` / `sync`
