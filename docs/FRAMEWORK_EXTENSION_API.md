...
这份文档对应后台前端工作台首页与顶部通知中心的真实接口。

当前后端已经按以下固定路径实现，前端可以直接把 mock 请求切换成真实请求：

- `GET /api/workbench/summary`
- `GET /api/workbench/trends`
- `GET /api/workbench/todos`
- `GET /api/workbench/activities`
- `GET /api/notifications/panel`

## 1. 通用约定

### 1.1 Base URL

当前接口已由后端直接提供 `/api` 前缀，无需再依赖网关补齐：
...
项目统一返回 `Result`，本次已补充 `success` 字段，前端可以直接按下列结构处理：
...
| Header          | 是否必填 | 说明                                                  |
| --------------- | -------- | ----------------------------------------------------- |
| `X-User-Id`     | 否       | 用于计算公告 `read` 状态                              |
| `X-Client-Type` | 否       | 用于按平台过滤公告，支持 `web / android / ios / mini` |

不传时：

- `X-Client-Type` 默认按 `web` 处理
- `X-User-Id` 不参与已读判断，通知默认按未读返回

## 2. 工作台首页

### 2.1 GET /api/workbench/summary
